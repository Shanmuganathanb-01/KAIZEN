...
export function KaizenLogo({
  ...
  showWordmark = true,
  showTagline = false,
  taglineText = "SMALL STEPS. REAL PROGRESS.",
  layout = "vertical",
  className = "",
  onClick,
}: KaizenLogoProps) {
  ...
  const gradId = `kaizenGrad_${id}`;
  const flareId = `kaizenFlare_${id}`;
  const glowId = `kaizenGlow_${id}`;

  useEffect(() => {
    if (autoPlay) {
      timerRef.current = setTimeout(() => setPlaying(true), 150);
    }
    return () => {
  ...
  const wordmarkFontSize = isHorizontal
    ? size > 50
      ? "1.5rem"
      : size > 32
      ? "1.15rem"
  ...
  return (
    <div
      className={`inline-flex ${
        isHorizontal ? "flex-row items-center gap-3" : "flex-col items-center gap-3"
      } cursor-pointer select-none group ${className}`}
      onClick={replay}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          replay(e as unknown as React.MouseEvent);
        }
      }}
      aria-label="KAIZEN logo - Click to replay formation animation"
    >
      {/* SVG Spiral Mark */}
      <div
      ...
      >
        ...
        >
          ...
          <path
            ...
            style={{
              strokeDasharray: 410,
              strokeDashoffset: playing ? 0 : 410,
              transition: playing
          ...
          <circle
            ...
            style={{
              opacity: playing ? 1 : 0,
              transform: playing ? "scale(1)" : "scale(0.2)",
              transformOrigin: "149.75px 26.75px",
              transition: playing
                ? "opacity 0.45s ease-out 1.25s, transform 0.45s ease-out 1.25s"
                : "none",
            }}
          />
          <circle
            ...
            style={{
              ...
              transform: playing ? "scale(1)" : "scale(0.2)",
              transformOrigin: "149.75px 26.75px",
              transition: playing ? "opacity 0.35s ease-out 1.3s" : "none",
            }}
          />
        </svg>
      </div>
      ...
      {(showWordmark || showTagline) && (
        <div className={`flex flex-col ${isHorizontal ? "items-start" : "items-center"}`}>
          {showWordmark && (
            <div
              ...
              style={{
                ...
                color: "#E9E6FF",
                textShadow:
                  "0 0 12px rgba(0,229,255,0.6), 0 0 24px rgba(255,46,154,0.4)",
              }}
            >
              {letters.map((l, i) => (
                <span
                  ...
                  style={{
                    ...
                    transition: playing
                      ? `opacity 0.4s cubic-bezier(.2,.8,.2,1) ${
                          1.35 + i * 0.05
                        }s, transform 0.4s cubic-bezier(.2,.8,.2,1) ${
                          1.35 + i * 0.05
                        }s`
                      : "none",
                  }}
          ...
          {showTagline && (
            <p
              style={{
                fontSize: isHorizontal ? "0.6rem" : "0.7rem",
                letterSpacing: "0.2em",
                color: "#8C86B8",
                opacity: playing ? 0.9 : 0,
                transform: playing ? "translateY(0)" : "translateY(4px)",
                transition: playing
                  ? "opacity 0.5s ease 1.75s, transform 0.5s ease 1.75s"
                  : "none",
                textTransform: "uppercase",
                fontFamily: "var(--font-jetbrains), monospace",
                marginTop: isHorizontal ? "-1px" : "4px",
              }}
            >
              {taglineText}
            </p>
